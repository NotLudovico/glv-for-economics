...
The adjacency matrix $A$ fixes the topology, with $A_{ij} = 1$ when nodes $i$ and $j$ are
...
closed, bounded set of equations. This does not make the scale finite, since the
...
The structural feature that matters is that \eqref{eq:dydtau}--\eqref{eq:dydtau-vec}
are closed in $y$, so the evolution of the relative fractions no longer references the
...
$x(t) = M(t)\,y(t)$. The divergence of $M$ is confined to this second part, where it never
affects the shape, and we stop the reconstruction wherever $M$ is still
representable.

This last step does not tame $M$ itself. Once
...
($t^\ast \approx 3$ versus $t^\ast \approx 18$ in the stable case), so the same amount
...
reconstruction $x = My$ rather than through the shape integration itself.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{rescaled_unscaled_mu_minus_2.png}
    \caption{Rescaled (\panel{left}) and reconstructed unscaled (\panel{right}) trajectories for
...
We want the model to behave like an economy, so its total size should grow without
...
remaining symbols are structural. Here $\gamma = \overline{z_{ij} z_{ji}}$ is the
...
\begin{figure}[htbp]
    ...
    per-topology $\mu_c^{\mathrm{th}}$, dashed line), for the exponential (\panel{top}) and
    regular (\panel{bottom}) graphs at mean degree $C \in \{10, 50, 150\}$. The shape
...
scaling with the horizon, which is exactly the $c = 0$ condition. It is read
as this onset rather than as the midpoint of the visible drop, which the inflated subcritical branch
...
\begin{figure}[h]
    ...
    a horizon-inflated quantity rather than a fixed level. Above the critical point
...
interest, is registered cleanly. The observable also comes at no extra cost, since it is a
...
Comparing the two estimators is not a trade-off, since the shape scalar $c$ is the sounder
...
located less cleanly. We run it only as an independent check on $c$, locating
...
is what makes this sweep possible at all, since it keeps the supercritical runs finite and
...
against $0.585$ for the drop) is not intrinsic, arising instead as an artifact of how the drop
...
double limit $1 \ll C \ll N$, where each node must see enough neighbours for its local
...
$C$, being slightly negative at small $C$, crossing zero, and clearly positive by
...
where the mean field is exact rather than toward it; the genuine limit is approached by
...
sits slightly below the mean-field value rather than above it. The large, $N$-sensitive
...
\begin{figure}[h]
    ...
    degree $C$ for the exponential (\panel{left}) and regular (\panel{right}) graphs, at disorder
