...
It is worth isolating the role of the degree distribution itself here. The
threshold depends on $\nu(g)$ only through its moments---in the noiseless limit
through $\langle g^2 \rangle$ alone---so the natural reference is a \emph{regular}
...
value of $1/2$, and it stays above the exponential curve at every $\sigma$, the
gap widening as the disorder grows. The reading is that degree heterogeneity
\emph{lowers} the cooperative threshold: a broad spread of connectivities lets
...
all other parameters fixed (Fig.~\ref{fig:c_sweep}) shows the gap \emph{growing}
monotonically with $C$---from near zero at small $C$ to $\approx 0.27$ at
$C = 150$. The reason is that at $N = 1000$ a connectivity of $C = 150$ is no
...
degrades. Increasing $C$ at fixed $N$ therefore moves \emph{away} from the regime
...
\begin{figure}[h]
    ...
    \caption{Gap $\mu_c^{\mathrm{emp}} - \mu_c^{\mathrm{th}}$ between the
    empirical (final-time drop) and theoretical critical points versus mean
    connectivity $C$, at $N = 1000$, $\sigma = 0.2$ on an exponential graph
    (mean $\pm$ std over realizations). The gap \emph{grows} with $C$: at fixed
    $N$, larger $C$ raises $C/N$ and erodes the sparse, locally tree-like
    structure the mean-field theory assumes.}
    \label{fig:c_sweep}
\end{figure}
...
degree. To make a sweep this broad affordable we use the shape-scalar estimator
of Section~\ref{sec:estimator-c}, far cheaper than the drop; at $N = 1000$ it
reproduces the $C$-dependence of Fig.~\ref{fig:c_sweep}, so the two estimators
agree on the trend. For the exponential graph the gap grows with $C$ at every
$N$, but at fixed $C$ it \emph{shrinks} with $N$---roughly halving with each
doubling of $N$---so the curves march down toward zero: the mean-field value is
...
correction as structural. The regular graph behaves oppositely: with no spread in
degree it carries almost no finite-size bias---identically zero at $\sigma = 0$,
where neither degree heterogeneity nor disorder is present, and only a small
residual at $\sigma = 0.4$ confined to low $C$. The finite-size correction to
$\mu_c$ is thus carried almost entirely by \emph{degree heterogeneity}: large and
$N$-sensitive for the broad exponential distribution, nearly absent for the
homogeneous one.
