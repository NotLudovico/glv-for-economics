...
\citep{roy2020,altieri2021}. Rare large excursions drive firms below the floor, and the state can
fall into a frozen attractor. A realization can freeze onto a fixed point, or keep
...
The magnitude, however, is not where the granular models fail; the higher moments are, and the
next section turns to them.
...
The first (D1) asks how the spread of firm volatilities depends on size. Divide each firm's
volatility by the mean volatility at its size, $\sigma_i/\bar\sigma(S)$, and the resulting
distribution is the same at every size, with a bounded tail. The model reproduces this
collapse onto a single size-independent shape, as in the data.

The second (D3) is the sharper one, and it turns on the central limit theorem. If a firm
were an aggregate of many independent parts, its growth would look more Gaussian the larger
it grew, so the excess kurtosis of its growth would fall toward zero with size. The data do
the reverse. Large firms stay at least as fat-tailed as small ones. The model reproduces
this anti-Gaussian trend. The excess kurtosis of the size-unmixed growth rises from about
three for the smallest firms to about six for the largest. A firm's growth here is not a
sum of independent contributions; it carries the correlations the interactions impose, and
that is what keeps the tails heavy as the firm grows.
...
There is also a reason the exponent is hard to pin to a single value. A firm's relative size
mean-reverts on a memory time $\tau$, the autocorrelation time of the chaotic state, so the
size--volatility slope measured over a horizon $h$ is not a single number but a function of
$h/\tau$, steep when $h$ is short compared with the memory and the firm has barely wandered
from its starting size, flattening towards zero once $h$ outgrows $\tau$ and that size is
forgotten. Figure~\ref{fig:growing-clock} traces this curve. The exponent declines with the
horizon and reaches the empirical band only at long horizons, but placing the data on this
axis would require the mean-reversion time of real firm sizes, which we do not have. We
therefore report $\beta$ at the fixed yearly protocol, where it is about four times too steep,
and leave its magnitude as an open gap rather than tuning the horizon to close it.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{growing_clock.png}
    \caption{The size--volatility exponent depends on the measurement clock. \panel{Left}: how
    individual firm sizes wander away from the mean firm (the pack) and revert to it over the
    memory time $\tau$, the autocorrelation time of the chaotic state. \panel{Right}: the fitted
    ...
    The exponent reaches the empirical band (shaded) only at long horizons; we lack the
    firm-level reversion time needed to fix where the data sits on this axis, so we report
    $\beta$ at the fixed yearly protocol.}
...
a between-firm effect, generated by the network of interactions rather than by anything
inside a single firm. With the interactions removed there is nothing left, no noise
carrying the fluctuations and no single-firm mechanism producing a tail. It is also the size of the
...
distribution is symmetric and fat-tailed; and switching off the disorder removes them all, so
their origin lies in the interactions.
