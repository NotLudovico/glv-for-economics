...
The disorder normalization deserves writing out, because it is what controls the
firm-growth statistics below. As introduced above the couplings are normalized by the
global mean degree $C$; on a heterogeneous graph a second, per-firm choice is available,
normalizing each firm's couplings by its own degree $k_i$,
\begin{equation}
    \alpha_{ij} = \frac{\mu}{C} + \frac{\sigma}{\sqrt{C}}\,z_{ij}
    \quad\text{(mean degree)},
    \qquad
    \alpha_{ij} = \frac{\mu}{k_i} + \frac{\sigma}{\sqrt{k_i}}\,z_{ij}
    \quad\text{(own degree)},
    \label{eq:normalizations}
\end{equation}
with $z_{ij}$ standard Gaussian and $k_i$ the degree of firm $i$. Under the mean-degree
normalization a firm's idiosyncratic field grows louder the more competitors it has, its
variance rising with its degree; the own-degree normalization is the natural per-fan-in
scaling, in which both the mean pressure and the disorder are shared over a firm's own
neighbours, so that every firm carries a field of the same variance whatever its
connectivity. The two coincide on a regular graph, where
