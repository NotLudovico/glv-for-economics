...
\begin{figure}[htbp]
    ...
    \eqref{eq:dmft-sc}--\eqref{eq:dmft-stability} at $\gamma=0$. \panel{Left}: the $(\mu,\sigma)$
    ...
    Section~\ref{sec:dmft-outlook}. \panel{Right}: the $\mu$-independent order parameters
...
character is already visible, however: the noise $\eta$ in \eqref{eq:dmft-singlesite} is a
...
\begin{figure}[htbp]
    ...
    fully-connected ensemble, $\mu = 0.5$, $\gamma = 0$, $N = 1500$. \panel{Left}: the growth rate
    tracks $g^\ast$ in the relaxed phase and the unstable-fixed-point branch (dotted)
    overshoots beyond $\sigma_c$. \panel{Centre}: the surviving fraction follows $\varphi$ up to
    $\sigma_c$. \panel{Right}: the persistent-fluctuation amplitude is negligible below
...
\eqref{eq:dmft-singlesite} with $\mu M_1 \to \kappa\mu M_1$, $\sigma \to \sqrt\kappa\,\sigma$
