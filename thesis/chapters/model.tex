...
louder the more competitors it has, its variance rising with its degree. The principled
alternative scales each firm's couplings by its own degree, $\sigma/\sqrt{k_i}$, so that
every firm carries a field of the same variance whatever its connectivity. The two
coincide on a regular graph, where every firm has the mean degree, which is the case the
mean-field theory of Appendix~\ref{app:dmft} solves; they differ on the heterogeneous
graphs we use for the measurements. The choice is consequential: the own-degree
normalization reproduces the higher-moment structure of the data, while the global one
matches the size--variance magnitude at a finite economy size, and no single choice gives
both. We take the own-degree normalization as the principled default for the firm-growth
statistics, and return to the choice, its trade-off, and the family that interpolates
between the two in Sections~\ref{sec:growing-finite-size} and~\ref{sec:growing-multiscaling}.
