...
Two cautions shape the measurement. The
...
comparisons we can make with the data. With the couplings normalized by the global mean
degree of the graph, as so far, the exponent is not fixed by the mean-field limit:
...
A principled alternative normalizes each firm's couplings by its own degree, so that
every firm carries a disorder field of the same variance whatever its connectivity, the
natural per-fan-in scaling on a heterogeneous graph. Under this own-degree normalization
the exponent is instead a clean power law that is robust across system size, a genuine
thermodynamic quantity, but its value is steeper, $\beta \approx 0.7$, about three times
the empirical one. The two normalizations therefore trade against each other on
...
growing band but not far from its $g_{\mathrm{eff}} = 0$ boundary. Finally, the model
reproduces the multiscaling of the higher moments, the signature \citet{moran2024} read as
correlations that grow with firm size; measuring that correlation structure directly,
